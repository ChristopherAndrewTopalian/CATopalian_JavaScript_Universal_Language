...
function processCodeString(rawCode, languageKey)
{
    ...
    // UNPACK THE BOX
    // We unpack backwards (Last-In, First-Out) to resolve nested placeholders
    for (let i = safeBox.length - 1; i >= 0; i--) 
    {
        // Using replaceAll ensures multiple instances are restored perfectly
        code = code.replaceAll(`__PH${i}__`, safeBox[i]);
    }
